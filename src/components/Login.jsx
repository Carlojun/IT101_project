...
const LoginPage = () => { 
  ...
  const handleSubmit = async () => {
    ...
    if (result.success) {
      // SUCCESS: App.jsx will detect the user change and redirect automatically.
      // We can also force a replace navigation to be safe.
      navigate('/', { replace: true });
    } else {
      // ERROR: Show actual Firebase error (e.g., "Wrong password")
      // Cleaning up the error message for better UX
  ...
  // --- 3. FORGOT PASSWORD (Mocked for now as it requires complex setup) ---
      ...
      <div className="auth-card">
        <div className="auth-grid">
          ...
          <div className="form-section">
            {error && <div className="error-msg" style={{backgroundColor: '#fee', color: '#c33', padding: '0.8rem', borderRadius: '8px', marginBottom: '1rem', textAlign: 'center'}}>{error}</div>}
            {successMessage && <div className="success-msg" style={{backgroundColor: '#d4edda', color: '#155724', padding: '0.8rem', borderRadius: '8px', marginBottom: '1rem', textAlign: 'center'}}>{successMessage}</div>}

            {isForgotPassword ? (
               /* Forgot Password UI (Same as before) */
               <>
                ...
                <div className="form-fields" style={{ marginTop: '20px' }}>
                  ...
                  {fpStep === 2 && (<><div className="input-group"><input type="text" placeholder="Enter 6-digit code" value={otp} onChange={(e) => setOtp(e.target.value)} className="input-field" maxLength={6} style={{ color: 'black', textAlign: 'center' }}/></div><button onClick={handleVerifyFpOtp} className="submit-button" disabled={loading} style={{ marginTop: '1rem' }}>{loading ? 'VERIFYING...' : 'VERIFY'}</button></>)}
