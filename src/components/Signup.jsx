      ...
      <div className="auth-card">
        <div className="auth-grid">
          ...
          <div className="form-section">
            {renderBackButton()}
            {error && <div style={{backgroundColor: '#fee', color: '#c33', padding: '0.8rem', borderRadius: '4px', marginBottom: '1rem', textAlign: 'center', fontSize: '0.9rem'}}>{error}</div>}
            ...
            <div className="form-fields">
              ...
              {step === 2 && (
                <>
                    ...
                    <div className="input-group"><label className="input-label">Verification Code</label><input type="text" placeholder="Enter 6-digit code" value={otp} onChange={(e) => setOtp(e.target.value)} className="input-field" style={{ color: 'black', letterSpacing: '4px', textAlign: 'center', fontSize: '1.2rem' }} maxLength={6} onKeyPress={(e) => e.key === 'Enter' && handleVerifyOtp()}/></div>
              ...
              {step === 3 && (
                <>
                    <div style={{ marginBottom: '1.5rem', textAlign: 'center', color: '#1a1a1a', fontSize: '0.9rem', fontWeight: 'bold' }}>✓ Phone Verified! Set your password.</div>
                    <div className="input-group"><label className="input-label">Create Password</label><div className="password-wrapper"><input type={showPassword ? "text" : "password"} placeholder="Enter password" value={password} onChange={(e) => setPassword(e.target.value)} className="input-field" style={{ color: 'black' }} onKeyPress={(e) => e.key === 'Enter' && handleSignup()}/><button type="button" className="password-toggle-icon" onClick={() => setShowPassword(!showPassword)} tabIndex="-1">{showPassword ? <FaEye size={20} /> : <FaEyeSlash size={20} />}</button></div></div>
              ...
              <p className="terms-text" style={{ textAlign: 'center', marginTop: '-1rem', fontSize: '10px' }}>By signing up, you agree to Sungla's <span className="terms-link" onClick={() => setShowTerms(true)} style={{ cursor: 'pointer', textDecoration: 'underline', fontWeight: 'bold' }}>Terms & Conditions</span></p>
